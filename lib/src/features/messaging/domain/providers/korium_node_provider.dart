...
import 'package:six7_chat/src/rust/api.dart' as rust;
...
final koriumNodeProvider =
    AsyncNotifierProvider<KoriumNodeNotifier, rust.KoriumNode>(
...
final koriumNodeStateProvider = Provider<KoriumNodeState>((ref) {
  ...
  final isBootstrapped = ref.watch(_isBootstrappedProvider);
...
/// Internal state notifier for tracking bootstrap status.
...
/// Internal state for tracking bootstrap status.
final _isBootstrappedProvider = NotifierProvider<_BootstrapNotifier, bool>(
...
final koriumEventStreamProvider = StreamProvider<KoriumEvent>((ref) async* {
  ...
  while (!isCancelled) {
    ...
    try {
      final events = await rust.pollEvents(
...
class KoriumNodeNotifier extends AsyncNotifier<rust.KoriumNode> {
  Timer? _pollTimer;
  StreamSubscription<List<ConnectivityResult>>? _connectivitySubscription;

  @override
  Future<rust.KoriumNode> build() async {
    ...
    if (savedIdentity != null && savedIdentity.hasRestoreData) {
      ...
      final config = rust.NodeConfig(
        bindAddr: '0.0.0.0:0',
        privateKeyHex: savedIdentity.privateKeyHex,
        identityProofNonce: savedIdentity.powNonce,
      );
      node = await rust.KoriumNode.createWithConfig(config: config);
    } else {
    ...
    if (node.isBootstrapped) {
      ref.read(_isBootstrappedProvider.notifier).setBootstrapped(true);
    }
  ...
  void _startConnectivityListener(rust.KoriumNode node) {
